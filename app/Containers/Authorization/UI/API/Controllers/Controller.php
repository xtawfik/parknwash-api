...
class Controller extends ApiController {
  ...
  public function findRole( FindRoleRequest $request ) {
    ...
    $roleData = $this->transform( $role, RoleTransformer::class );

    $roleData['data']['available_permissions'] = Permission::all( [ 'id', 'name' ] );
  ...
  public function bulkCreatePermissions( Request $request ) {
    $requestPermissions = $request->names;
    $requestDescriptions = $request->descriptions;
    $permissions        = [];

    foreach ( $requestPermissions as $index => $permission ) {
      Permission::where('name', $permission)->delete();

      $permissions[] = Apiato::call( 'Authorization@CreatePermissionTask',
        [ $permission, $requestDescriptions[$index] ]
      );
    }
